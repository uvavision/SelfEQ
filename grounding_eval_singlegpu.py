...
#     config['test_file'] = ['/vislang/ziyan/data/refcoco+_val_new_path.json','/vislang/ziyan/data/refcoco+_test_new_path.json']
    ...
    if os.path.isdir(args.checkpoint):
        ...
        for checkpoint in all_:
            ...
            grounding_acc, mean_bbox = grounding_eval(result, dets, refer, alpha=0.5, mask_size=24, on_bbox = True)
            ...
            grounding_acc, mean_point = grounding_eval(result, dets, refer, alpha=0.5, mask_size=24, on_bbox = False)
            log_stats = {**{f'{k}': v for k, v in grounding_acc.items()},
                             'epoch': checkpoint[-6:-4],
                            } 
            with open(os.path.join(args.result_dir, "log.txt"),"a") as f:
